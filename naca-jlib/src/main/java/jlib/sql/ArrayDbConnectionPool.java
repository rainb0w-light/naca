...
public class ArrayDbConnectionPool
{
    ...
    synchronized void handleCleanings(
        MemoryPoolMXBean tenuredPool,
        int nNbStatementsToRemoveBeforeGC,
        int nNbStatementForcedRemoved,
        int nNbSystemGCCall)
    {
        if(dbConnectionPool != null)
        {
            ...
            if(tenuredPool != null && tenuredPool.isUsageThresholdExceeded() && nNbStatementForcedRemoved > 0)
            {
                ...
                nNbStatementAggressiveRemoved = aggressiveRemoveObsoleteStatements(
                    tenuredPool,
                    mapStatements,
