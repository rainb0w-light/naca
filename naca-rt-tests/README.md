...
| `BATCH1.cbl`, `CALLMSG.cbl`, `MSGZONE` | Generated by `BatchMultiProgramAcceptanceTest` | Exact: dynamic CALL result, three input records, and two output records are verified |
...
- `TEST-A-STANDALONE.cbl`: GnuCOBOL and NacaRT must match all 34 canonical lines.
- `BATCH1.cbl` + `CALLMSG.cbl` + `MSGZONE`: Naca transpiles all sources, `javac`
  compiles them together, NacaRT dynamically calls CALLMSG by reference, and the
  test verifies exact business output and FILEOUT records from a fixed-record input.
