...
public class CCondDifferentStatement extends CExpression
{
    ...
    public CBaseEntityCondition AnalyseCondition(CBaseEntityFactory factory, CDefaultConditionManager masterCond)
    {
        ...
        {
            CBaseEntityCondition eCond = eData1.GetSpecialCondition(
                getLine(),
                eData2,
                CBaseEntityCondition.EConditionType.IS_DIFFERENT,
                factory) ;
            if (eCond != null)
            {
        ...
        if (eData != null)
        {
            CBaseEntityCondition eCond = eData.GetSpecialCondition(
                getLine(),
                value,
                CBaseEntityCondition.EConditionType.IS_DIFFERENT,
                factory) ;
