...
public class CExecSQLSelect extends CBaseExecSQLAction
{
    ...
    static String PrepareSelectStatement(
        CBaseLanguageEntity parent,
        String clause,
        Vector<String> arrColumns,
    ...
    static String PrepareSelectStatementJoin(
        CBaseLanguageEntity parent,
        String clause,
        CBaseEntityFactory factory,
        boolean bCursor,
    ...
    static String CheckConcat(int nLine, String clause, Vector arrParameters, Vector<CDataEntity> arrParam, CBaseEntityFactory factory)
    {
        for (int i=0; i<arrParameters.size(); i++)
        {
            CIdentifier id = (CIdentifier)arrParameters.get(i);
     ...
     * @param arrInto2
     */
    private static void processInto(
        Vector<CIdentifier> arrIntoInput,
        Vector<CDataEntity> arrIntoOutput,
        int nbCol,
        CBaseEntityFactory factory)
    {
    ...
    public static String ManageFrom(CBaseLanguageEntity parent, String from, CBaseEntityFactory factory, boolean bCursor)
    {
        ...
        do
        {
            ...
            String tablename = cs;
            CEntitySQLDeclareTable table = factory.programCatalog.GetSQLTable(cs) ;
            if (table != null)
            {
            ...
            else
            {
                if (factory.programCatalog.getProcedureDivision() != null)
                {
                    CGlobalEntityCounter.GetInstance().RegisterProgramToRewrite(
                        factory.programCatalog.getProcedureDivision().GetProgramName(),
                        0,
                        "Missing table declaration : "+cs);
                }
