...
public class CInspect extends CCobolElement
{
    ...
    private CBaseLanguageEntity analysis(CBaseLanguageEntity parent, CBaseEntityFactory factory, CInspectAction a)
    {
        ...
        if (method == CInspectActionType.REPLACING)
        {
            ...
            {
                CInspectValueToReplace item = itemToReplace.get(i);
                CTerminal term ;
                ...
                else if (term.GetValue().equals(CCobolConstantList.ZERO.name) || term.GetValue().equals(CCobolConstantList.ZEROS.name)
                    || term.GetValue().equals(CCobolConstantList.ZEROES.name))
                {
                    eReplace.ReplaceZeros() ;
                }
                else if (term.GetValue().equals(CCobolConstantList.LOW_VALUE.name)
                    || term.GetValue().equals(CCobolConstantList.LOW_VALUES.name))
                {
                    eReplace.ReplaceLowValues();
                }
                else if (term.GetValue().equals(CCobolConstantList.HIGH_VALUE.name)
                    || term.GetValue().equals(CCobolConstantList.HIGH_VALUES.name))
                {
                ...
                // value to replace by
                if (item.valNew.GetValue().equals(CCobolConstantList.SPACE.name)
                    || item.valNew.GetValue().equals(CCobolConstantList.SPACES.name))
                {
                    eReplace.BySpaces() ;
                }
                else if (item.valNew.GetValue().equals(CCobolConstantList.ZERO.name)
                    || item.valNew.GetValue().equals(CCobolConstantList.ZEROS.name)
                    || item.valNew.GetValue().equals(CCobolConstantList.ZEROES.name))
                {
                    eReplace.ByZeros() ;
                }
                else if (item.valNew.GetValue().equals(CCobolConstantList.LOW_VALUE.name)
                    || item.valNew.GetValue().equals(CCobolConstantList.LOW_VALUES.name))
                {
                    eReplace.ByLowValues();
                }
                else if (item.valNew.GetValue().equals(CCobolConstantList.HIGH_VALUE.name)
                    || item.valNew.GetValue().equals(CCobolConstantList.HIGH_VALUES.name))
                {
        ...
        else if (method == CInspectActionType.TALLYING)
        {
            ...
            Vector<CInspectItemToCount> arrItemToCount = a.itemToCount;
            for (int i=0; i<arrItemToCount.size();i++)
            {
                ...
                for (CInspectItem item : itemToCount.items)
                {
                    for (int j=0; j<item.tokenToCount.size(); j++)
                    {
                        ...
                        else if (item.ischaractersAfter)
                        {
                            eCount.CountAfter(t.GetDataEntity(getLine(), factory));
                        }
                        else if (item.ischaractersBefore)
                        {
            ...
            while (!isdone2)
            {
                ...
                if (tok.GetKeyword() == CCobolKeywordList.FOR)
                {
                    ...
                    if (tok.GetKeyword() == CCobolKeywordList.CHARACTERS)
                    {
                        ...
                        {
                            count.ischaractersAfter = true ;
                        }
                        else if (tok.GetKeyword() == CCobolKeywordList.BEFORE)
                        {
                            count.ischaractersBefore = true ;
                        }
                        else
                        {
                        ...
                        while (!isdone)
                        {
                            ...
                            if (!isdone)
                            {
                                ...
                                while (t != null)
                                {
                                    ...
                                    if (tok.GetType() == CTokenType.STRING || tok.GetType() == CTokenType.CONSTANT
                                ...
                                if (tok.GetKeyword() == CCobolKeywordList.AFTER || tok.GetKeyword() == CCobolKeywordList.BEFORE)
                                {
                                    ...
                                    if (tok.GetKeyword() == CCobolKeywordList.AFTER)
                                    {
                                        item.ischaractersAfter = true ;
                                    }
                                    else if (tok.GetKeyword() == CCobolKeywordList.BEFORE)
                                    {
                                        item.ischaractersBefore = true ;
                                    }
                                    tok = GetNext();
                                    if (tok.GetKeyword() == CCobolKeywordList.INITIAL)
                                    {
    ...
    {
        boolean isall = false ;
        boolean ischaractersBefore = false ;
        boolean ischaractersAfter = false ;
