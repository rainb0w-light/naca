...
public class CWorkingEntry extends CCobolElement
{
    ...
    protected boolean ParsePicOptions()
    {
        ...
        while (!isdone)
        {
            ...
            else if (tokPic.GetKeyword()==CCobolKeywordList.COMP_5)
            {
                comp = "COMP5" ;
                isnext = true ;
                GetNext();
            }
            else if (tokPic.GetKeyword()==CCobolKeywordList.COMP || tokPic.GetKeyword()==CCobolKeywordList.COMPUTATIONAL)
            {
                comp = "COMP" ;
                isnext = true ;
                GetNext();
            ...
            else if (tokPic.IsKeyword() && tokPic.GetKeyword()==CCobolKeywordList.BLANK)
            {
                ...
                if (tok.GetConstant() == CCobolConstantList.ZERO || tok.GetConstant() == CCobolConstantList.ZEROS
                    || tok.GetConstant() == CCobolConstantList.ZEROES)
                {
                    tok = GetNext();
    ...
    protected CBaseLanguageEntity DoCustomSemanticAnalysis(CBaseLanguageEntity parent, CBaseEntityFactory factory)
    {
        ...
        if (value != null)
        {
            ...
            else if (value.GetValue().equals(CCobolConstantList.ZERO.name) || value.GetValue().equals(CCobolConstantList.ZEROS.name)
                || value.GetValue().equals(CCobolConstantList.ZEROES.name))
            {
                eAtt.SetInitialValueZeros() ;
            }
            else if (value.GetValue().equals(CCobolConstantList.LOW_VALUE.name)
                || value.GetValue().equals(CCobolConstantList.LOW_VALUES.name))
            {
                eAtt.SetInitialLowValue() ;
            }
            else if (value.GetValue().equals(CCobolConstantList.HIGH_VALUE.name)
                || value.GetValue().equals(CCobolConstantList.HIGH_VALUES.name))
            {
    ...
    protected int DoSemanticAnalysisForMapRedefineForChildren(
        CEntityResourceForm eForm,
        CBaseEntityFactory factory,
        CBaseLanguageEntity eParent,
        boolean bSaveMap,
        CEntityResourceForm.CFieldRedefineStructure structure)
    {
        ...
        Hashtable<CBaseLanguageEntity, CEntityResourceForm.CFormByteConsumingState> tabPassedStates
                = new Hashtable<CBaseLanguageEntity, CEntityResourceForm.CFormByteConsumingState>() ;
        while (le != null)
        {
...
// CBaseTranscoder.ms_logger.info("INFO : Data field splitted into sub-fields, line" + le.getLine()) ;
                            ...
                            if (curRedefineStructure.field != null)
                            {
                                if (curRedefineStructure.size != 1)
                                {
                                    ...
                                    throw new NacaTransAssertException("ERROR : unexpected situation while analysing MAP REDEFINE, line "
                                        + le.getLine()) ; // ASSERT
                                }
                            }
                            else
...
//                      CBaseTranscoder.ms_logger.info("INFO : group of fields, line" + le.getLine()) ;
     ...
     * @return
     */
    private CEntityFieldRedefine CheckRadical(
        CEntityFieldRedefine fieldRedef,
        CWorkingEntry le,
        CBaseLanguageEntity eParent,
        CBaseEntityFactory factory,
        CFieldRedefineDescription curRedefineStructure,
        boolean bSaveMap)
    {
        String csRadical = "" ;
        if (!le.name.equals(""))
     ...
     * @return
     */
    private boolean CreateAttributeForCurrentPositionInField(
        CWorkingEntry le,
        CEntityResourceForm eForm,
        CEntityResourceField eField,
        CBaseEntityFactory factory)
    {
        String name = le.name ;
