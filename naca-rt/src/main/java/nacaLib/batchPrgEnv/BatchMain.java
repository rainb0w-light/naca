        ...
        try
        {
            ...
            else
            {
                ...
                if (nPos != -1)
                {
                    ...
                    nGeneration = Integer.valueOf(csParameter.substring(nPos + 12)).intValue();
                }
...
//          }
    ...
    private static void doRunBatchProgram(
        String csConfigFile,
        String csDB,
        boolean bEnableInitialConnectDb,
        String csParameter,
        String csPrgClassName)
    {
        // Batch program
        BatchResourceManager batchResourceManager = BatchResourceManagerFactory.GetInstance(csConfigFile, csDB);
        BaseEnvironment env = null;
        try
        {
            ...
            if(iscanExecute)
            {
                if (csParameter != null)
                {
                    InternalCharBuffer charBuffer = new InternalCharBuffer(2 + csParameter.length());
                    int nPos = 0;
                    nPos = charBuffer.writeShort(Integer.valueOf(csParameter.length()).shortValue(), nPos);
                    nPos = charBuffer.writeString(csParameter, nPos);
                    CCommarea comm = new CCommarea();
                    comm.setVarPassedByValue(charBuffer);
                    env.setCommarea(comm);
    ...
    static void displayHelp()
    {
        ...
        System.out.println("\tLast argument must be \"SORT FIELDS=(1 Based position,Length,CH|PD|BI|FI,A|D)+\"\t// Describe the sort key " +
            "e.g. : \"SORT FIELDS=(1,8,CH,A,9,4,PD,A,13,2,CH,A)\"");
    }
}
