...
public class TableToTransfer extends ThreadPoolRequest
{
    ...
    private void doTransfers(DbConnectionBase dbConnectionSource, DbConnectionBase dbConnectionDestination, DbTransferDesc dbTransferDesc)
    {
        ...
        if(selectStatement != null)
        {
            ...
            if(resultSet != null)
            {
                try
                {
                    ...
                    while(resultSet.next())
                    {
                        if(nNbRecordRead == 0)
                        {
                            ...
                            dbInsertStatement = getInsertStatement(
                                dbConnectionDestination,
                                resultSet,
                                nNbColumns,
                                resultSetMetaData,
                                collectionnames);
                        ...
                        for(int nColumn=0; nColumn<nNbColumns; nColumn++)
                        {
                            ...
                            if(!iscol)
                            {
                                String cs = "Problem during transfer of a column. Source table=" + csPrefixedTableName
                                    + "; Destination table=" + csDestinationTableName + "; Column name=" + collectionnames.get(nColumn)
                                    + "; Clause=" + csClause + "; Record number=" + nNbRecordRead;
                                Log.logCritical(cs);
                                sbSQLError = appendIfPossible(sbSQLError, cs);
                            }
                            b |= iscol;
                        ...
                        if(b)
                        {
                            ...
                            {
                                SQLLoadStatus status = doWriteBatchRecords(
                                    dbInsertStatement,
                                    nNbRecordRead,
                                    nBatchSize,
                                    sbSQLError,
                                ...
                                else
                                {
                                    ...
                                    String cs = "Execute batch insert error for records. Status=" + status.toString() + "; Source table="
                                        + csPrefixedTableName + "; Destination table=" + csDestinationTableName + "; Record number range"
                                        + nMin + ", " + nNbRecordRead + "]";
                                    Log.logCritical(cs);
                    ...
                    if(dbInsertStatement != null)
                    {
                        ...
                        else
                        {
                            ...
                            String cs = "Execute batch insert error for records. Status=" + status.toString() + "Source table="
                                + csPrefixedTableName + "; Destination table=" + csDestinationTableName + "; Record number range" + nMin
                                + ", " + nNbRecordRead + "]";
                            Log.logCritical(cs);
                            sbSQLError = appendIfPossible(sbSQLError, cs);
                            dbTransferDesc.setTransferGlobalFailure();
                ...
                {
                    String cs = "Exception catched during transfer. Source table=" + csPrefixedTableName + "; Destination table="
                        + csDestinationTableName + "; Clause=" + csClause + "; Exception catched=" + e.getMessage();
                    sbSQLError = appendIfPossible(sbSQLError, cs);
                    Log.logCritical(cs);
                    dbTransferDesc.setTransferGlobalFailure();
                }
    ...
    private SQLLoadStatus doWriteBatchRecords(
        DbPreparedStatement dbInsertStatement,
        int nNbRecordRead,
        int nBatchSize,
        StringBuilder sbSQLError,
        String csPrefixedTableName,
        String csDestinationTableName)
    {
        int nMin = nNbRecordRead-nBatchSize;
        SQLLoadStatus status = dbInsertStatement.executeBatch(nMin);
        if(!status.isSuccess())
        {
            String cs = "Execute batch insert error for records. Status=" + status.toString() + "; Source table=" + csPrefixedTableName
                + "; Destination table=" + csDestinationTableName + "; Record number range" + nMin + ", " + nNbRecordRead + "]";
            Log.logCritical(cs);
    ...
    private synchronized void reportTransfer(
        ...
        String csDestinationTableName)
    {
        ...
        Log.logCritical("Finished transfer from source table " + csPrefixedTableName + " to destination table " + csDestinationTableName
            + "; Records read=" + nNbRecordRead + "; Records written=" + nNbRecordWritten + "; Transfet Time=" + sw.getElapsedTime()
            + " ms; Text error=" + csError);
    ...
    private DbPreparedStatement getInsertStatement(
        DbConnectionBase dbConnectionDestination,
        ResultSet resultSet,
        int nNbColumns,
        ResultSetMetaData resultSetMetaData,
        ArrayList<String> arrColNames)
    {
        try
        {
    ...
    private ArrayList<RecordColTypeManagerBase> getColumnsTypes(int nNbColumns, ResultSetMetaData resultSetMetaData)
    {
        ...
        try
        {
            ...
            for(int nColSourceIndex=1; nColSourceIndex<=nNbColumns; nColSourceIndex++)
            {
                ...
                // LONG is for ORACLE Support
                else if(csColTypeName.equals("VARCHAR") || csColTypeName.equals("LONG VARCHAR") || csColTypeName.equals("LONG"))
                {
