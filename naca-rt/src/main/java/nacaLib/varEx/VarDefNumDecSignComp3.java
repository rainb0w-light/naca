...
public class VarDefNumDecSignComp3 extends VarDefNum
{
    ...
    /** Creates a new var def num dec sign comp3 instance. */
...
//      return n;
    ...
    CStr getDottedSignedString(VarBufferPos buffer)
    {
        ...
        int totalDigits = nNbDigitInteger + nNbDigitDecimal;
        int displayIntegerWidth = nNbDigitInteger + (totalDigits % 2 == 0 ? 1 : 0);
        return formatPicture(dec.getUnsignedLong(), dec.getDecPart(), hasNegativeComp3Sign(buffer),
            displayIntegerWidth, nNbDigitDecimal, true, false);
    }
...
//  }
    ...
    public void initializeAtOffset(VarBufferPos buffer, int nOffset, InitializeCache initializeCache)
    {
        Dec dec = new Dec(0L, "");
        // Only unsigned dec
        Pic9Comp3BufferSupport.setDec(buffer, nNbDigitInteger, nNbDigitDecimal, nTotalSize, nOffset, true, dec);
