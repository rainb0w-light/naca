...
public class NacaToolBox extends CJMapObject
{
    ...
    private String completeDate(GregorianCalendar gc, boolean bLongInfo)
    {
        ...
        if (bLongInfo)
        {
            return csDate + csDayOfWeek + csDaysInMonth + csDayOfYear + csDaysToEndYear + csDaysInYear + csWeekOfYear + csDayEaster
                + csFlagHoliday;
        }
        else
        {
...
// Var s32_Ichar = declare.level(15).occurs(4001).picX(1).var() ; // (2501) 15 S32-ICHAR OCCURS 4001 PIC X.
        ...
        if (blockMode.getInt() == 1)
            rc = textPrintHelper.printText(input.getVarChildAt(3).getString().substring(
                0,
                input.getVarChildAt(1).getInt()), TextBlockMode.BLKM_MORE, psf);
        else
            rc = textPrintHelper.printText(input.getVarChildAt(3).getString().substring(
                0,
                input.getVarChildAt(1).getInt()), TextBlockMode.BLKM_ENDOFBLOCK, psf);
...
// Var s33_Cyline = declare.level(10).picS9(9).comp().var() ; // (2512) 10 S33-CYLINE PIC S9(9) COMP.
        /*LineDef lineDef = new LineDef(input.getVarChildAt(1).getInt(), input.getVarChildAt(2).getInt(), input.getVarChildAt(3).getInt(),
        input.getVarChildAt(4).getInt());
