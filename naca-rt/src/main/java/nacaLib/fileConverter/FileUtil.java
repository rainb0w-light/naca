...
public class FileUtil
{
    ...
    public boolean execute(String csParameter)
    {
        ...
        while (true)
        {
            ...
            if (nCount != -1 && (nReplace == -1 || nCount < nReplace) && (nDelete == -1 || nCount < nDelete) && (nExtract == -1
                || nCount < nExtract))
            {
                iscount = true;
                nPosStart = nCount + 1;
            }
            else if (nReplace != -1 && (nCount == -1 || nReplace < nCount) && (nDelete == -1 || nReplace < nDelete) && (nExtract == -1
                || nReplace < nExtract))
            {
                isreplace = true;
                nPosStart = nReplace + 1;
            }
            else if (nDelete != -1 && (nCount == -1 || nDelete < nCount) && (nReplace == -1 || nDelete < nReplace) && (nExtract == -1
                || nDelete < nExtract))
            {
                isdelete = true;
                nPosStart = nDelete + 1;
