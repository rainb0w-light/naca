...
//  private static final byte[] AFP_EBCDIC_IOB          = { (byte)0xD3, (byte)0xAF, (byte)0xC3 };
    ...
    public boolean execute(String csParameter)
    {
        ...
        } else if (issuppressVariableLength) {
            ...
            if (nLengthLine > 6 && arrByteValue[0] == AFP_ASCII_5A)
            {
                if (isSpecialAfp(arrByteValue, AFP_ASCII_COPYGROUP) || isSpecialAfp(arrByteValue, AFP_ASCII_PAGEFORMAT))
                {
                    ...
                    if (nLengthAFP != 16)
                    {
                        ...
                        Log.logDebug("FileConverter: Change the record x'5a' length copygroup/pageformat at line " + nLine
                            + " for document " + dataFileIn.getName());
                    }
                }
                else if (isSpecialAfp(arrByteValue, AFP_ASCII_SEGMENT))
                {
                    ...
                    if (nLengthAFPSegment != 22)
                    {
                        ...
                        Log.logDebug("FileConverter: Change the record x'5a' length segment at 22 in line " + nLine + " for document "
                            + dataFileIn.getName());
                    }
                }
                ...
                {
                    Log.logCritical("FileConverter: No transformation ebcdic for the record x'5a' in line " + nLine + " for document "
                        + dataFileIn.getName());
                }
            }
