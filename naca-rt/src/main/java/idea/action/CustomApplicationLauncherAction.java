...
//			}
	...
	private String GetValFromField(String field, OnlineSession appSession)
	{
		...
		if (posSubstr != -1)
		{
			...
			if (posSecond == -1)
			{
				beginIndex = Integer.valueOf(field.substring(posFirst + 1, posEnd).trim()).intValue();
			}
			else
			{
				beginIndex = Integer.valueOf(field.substring(posFirst + 1, posSecond).trim()).intValue();
				endIndex = Integer.valueOf(field.substring(posSecond + 1, posEnd).trim()).intValue();
			}
		}
		else
		{
